...
impl AppServiceImpl {
    ...
    fn with_inner<R>(&self, f: impl FnOnce(&InnerState) -> R) -> R {
        let guard = lock_inner(&self.inner);
        f(&guard)
    }

    fn with_inner_mut<R>(&self, f: impl FnOnce(&mut InnerState) -> R) -> R {
    ...
    async fn handle_command(&self, cmd: AppCommand) -> Result<(), AppError> {
        match cmd {
            ...
            AppCommand::RestoreSession => {
                ...
                self.with_inner_mut(|inner| match session {
                    Some(session) => {
                        inner.app_state.auth_state = AuthState::LoggedIn {
                            user_id: session.user_id,
                        };
            ...
            AppCommand::StopMonitor => {
                self.stop_monitor_engine().await;
                self.with_inner_mut(|inner| inner.app_state.monitor_running = false);
                self.emit_state_changed().await;
                Ok(())
            }
